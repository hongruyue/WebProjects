...
//            dom.ontouchstart = onclick;
    ...
    getMenuItems: function(graph, data, evt){
      ...
      if (data) {
        ...
        if(isNode){
        	 /*if(data.image.indexOf("drawable-1")==0){
        	    	items.push({
                        text: getI18NString('Rename'), action: function (evt, item) {
                         ...
                         var select_Btn = document.getElementById("select_Btn");
                  	   	 ...
                  	   	 var selectName = $("#select_Depart option:selected").val();
                         alert(selectName)
                  	   		 data.name = selectName;
                  	   		 $.ajax({
                  	          		  type: "POST", 
                  	                  url: "/requirements/page/task/addopraname.action?",               
                  	                  data: "opername="+selectName, 
                  	                  success: function(data){
                  	               	   	alert("添加成功")
                  	                  	$('#myModal').modal('hide');
                  	               	   	//$("#select_Depart").removeAttr(option);
                  	                  },
                  	                  error:function(data){
                  	                  		alert("保存失败")
                  	                  }
...
//              data.setStyle(Q.Styles.EDGE_LINE_DASH, isDashLine ? null : [8,10]);
//            }
//          });
          /*
          items.push({
       ...
       /* items.push(Q.PopupMenu.Separator);
        items.push({
            text: getI18NString('EditDepart'), action: function (evt, item) {
            	window.parent.location.href="../BusinessName1/departmentManagement/departmentInformation.jsp";
               //graph.invalidate();
            }
  	     });*/
       /* items.push({
            text: getI18NString('DeleteNode'), action: function (evt, item) {
              //window.parent.location.href="../BusinessName1/departmentManagement/departmentInformation.jsp";
              data.zIndex = 1;
              graph.removeSelectionByInteraction();
            }
  	     })*/
       /* items.push({
