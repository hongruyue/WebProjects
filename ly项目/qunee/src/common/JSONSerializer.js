...
//            continue;
	...
	//添加代码(添加的userId属性不能添加到和refId同一层)
	//var OUTPUT_PROPERTIES = ['userId', 'enableSubNetwork', 'zIndex', 'tooltipType', 'tooltip', 'movable', 'selectable', 'resizable', 'uiClass', 'name', 'parent', 'host'];
    ...
    JSONSerializer.prototype = {
        ...
        toJSON: function (value) {
            ...
            if (value instanceof Q.Element) {
                ...
                this._elementRefs[value.id] = true;
               	//指定线的from和to的id,默认使用画布上随机的id(可以换成userId)
                return {_ref: value.id};
                //添加代码
                //var userIdOrId = getElementId(value);
				//由于禁调了部分代码所以map:_elementRefs暂时无用（670行代码）
				//this._elementRefs[userIdOrId] = true;
				//return {_ref: userIdOrId};
        ...
        _parseJSON: function (json) {
            ...
            if (json._className) {
                var F = getByPath(json._className);
                var v = new F();
                //将json数据的真实userId，传给新创建的对象v，并创建一个属性userId
            	if(json.userId) {
            		v.userId = json.userId;
            	} else {
            		v.userId = null;
            	}
                ///防止相互引用导致的问题
        ...
        model.forEach(function (d) {
            ...
            if (elementJson) {
                ...
                map[d.id] = elementJson;
             	//添加代码
                map[d.id]._refId = d.id;
                if(d.userId) {
            		map[d.id].userId = d.userId;
            	} else {
            		map[d.id].userId = 0;
            		//map[d.id].userId = undefined;
           	 	} 
            }
        });
        //功能移到上面的循环语句里了
